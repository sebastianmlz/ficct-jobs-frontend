...
export class AppShellComponent implements OnInit {
  ...
  protected readonly nav = computed<NavItem[]>(() => {
    ...
    if (role === 'candidate') {
      return [
        ...
        { label: 'Postulaciones', path: '/applications', icon: 'clipboard-check' },
    ...
    if (role === 'company_rep') {
      return [
        ...
        { label: 'Procesos cerrados', path: '/closed-history', icon: 'archive' },
        { label: 'Asistente', path: '/chat', icon: 'chat' },
      ];
    }
    if (role === 'admin_ficct') {
      return [
        ...
        { label: 'Revisión de candidatos', path: '/admin/candidates', icon: 'users' },
        { label: 'Empresas', path: '/admin/companies', icon: 'building' },
        { label: 'Cola de vacantes', path: '/admin/vacancies', icon: 'clipboard-list' },
        { label: 'Procesos cerrados', path: '/closed-history', icon: 'archive' },
  ...
  ngOnInit(): void {
    this.auth.fetchMe().subscribe({ next: () => undefined, error: () => undefined });
  }
