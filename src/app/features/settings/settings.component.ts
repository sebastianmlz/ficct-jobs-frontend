    ...
    this.auth.deleteAccount(password).subscribe({
      ...
      error: (err) => {
        ...
        if (code === "wrong_password")
          this.errorSignal.set("Contraseña incorrecta.");
        else if (code === "has_active_vacancies")
          this.errorSignal.set(
            "Cierre sus vacantes activas antes de eliminar la cuenta.",
          );
        else
          this.errorSignal.set(
            err?.error?.error?.message ?? "No se pudo eliminar la cuenta.",
          );
      },
    });
  }
