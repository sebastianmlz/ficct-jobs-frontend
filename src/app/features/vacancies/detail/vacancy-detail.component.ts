...
import { ApiErrorEnvelope, GapAnalysisResult, Vacancy } from '../../../core/models';
...
export class VacancyDetailComponent implements OnInit {
  ...
  protected readonly isCandidate = computed(() => this.auth.role() === 'candidate');
  protected readonly coverLetter = new FormControl<string>('', { nonNullable: true });
  ...
  protected apply(): void {
    ...
    this.jobs.apply(v.id, this.coverLetter.value).subscribe({
      next: () => { this.appliedSignal.set(true); this.busySignal.set(false); },
      error: (err) => {
        const envelope = err?.error as ApiErrorEnvelope | undefined;
        this.applyErrorSignal.set(envelope?.error?.message ?? 'No se pudo enviar la postulación. Intente de nuevo.');
  ...
  protected runGap(): void {
    ...
    this.intelligence.gap(v.id).subscribe({
      next: (result) => { this.gapSignal.set(result); this.gapBusySignal.set(false); },
