...
export class ChatComponent {
  ...
  protected readonly promptCtrl = new FormControl('', { nonNullable: true, validators: [Validators.required, Validators.minLength(2)] });
  ...
  protected ask(): void {
    ...
    if (!message) return;
    this.turnsSignal.update((t) => [...t, { role: 'user', content: message, at: Date.now() }]);
    ...
    this.intel.chat(message).subscribe({
      next: (response: ChatResponse) => {
        this.turnsSignal.update((t) => [
          ...t,
          { role: 'assistant', content: response.answer || '(sin respuesta)', sources: response.sources, at: Date.now() },
        ]);
        this.busySignal.set(false);
      },
      error: (err) => {
        this.busySignal.set(false);
        this.toast.danger('Asistente no disponible', err?.error?.error?.message ?? 'Intente de nuevo más tarde.');
      },
    });
  }
}
