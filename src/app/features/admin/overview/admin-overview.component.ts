...
export class AdminOverviewComponent implements OnInit {
  ...
  protected readonly stages = computed(() => {
    ...
    return Object.entries(s.applications_by_stage).map(([stage, count]) => ({
      stage,
      count,
    }));
  });
  protected readonly tokenOperations = computed(() => {
    ...
    return Object.entries(t.by_operation).map(([operation, row]) => ({
      operation,
      ...row,
    }));
  });
  ...
  protected readonly healthEntries = computed(() => {
    ...
    return Object.entries(h.body).map(([k, v]) => [
      k,
