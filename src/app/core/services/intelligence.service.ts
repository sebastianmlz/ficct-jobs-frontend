...
export class IntelligenceService {
  ...
  recommendations(topK = 5): Observable<RecommendationsResponse> {
    return this.api.get<RecommendationsResponse>('/intelligence/recommendations/', { top_k: topK });
  }

  affinityDetail(applicationId: string): Observable<AffinityDetail> {
    return this.api.get<AffinityDetail>(`/intelligence/affinity-detail/${applicationId}/`);
  }
}
